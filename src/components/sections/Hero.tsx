...
function NameDecode({ text, onComplete }: { text: string; onComplete?: () => void }) {
  ...
  useEffect(() => {
    if (reduced) { setDisplay(text); onComplete?.(); return; }
    let frame = 0;
    ...
    const animate = () => {
      ...
      const result = letters.map((ch, i) => {
        if (ch === ' ') return ' ';
        const cp = Math.max(0, (progress * letters.length - i) / 3);
        if (cp >= 1) return ch;
        if (cp > 0.5) return ch;
...
// Particle field — uses refs for mouse (no re-renders)
function ParticleField() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Connections (desktop only, throttled)
      if (!isMobile) {
        for (let i = 0; i < particles.length; i++) {
          const p = particles[i];
          const pdx = mx - p.x;
          const pdy = my - p.y;
          if (pdx * pdx + pdy * pdy > 22500) continue; // 150^2
          for (let j = i + 1; j < particles.length; j++) {
            const p2 = particles[j];
            const ddx = p.x - p2.x;
            const ddy = p.y - p2.y;
            const d = ddx * ddx + ddy * ddy;
            if (d < 10000 && d > 0) { // 100^2
              ctx.beginPath();
              ctx.moveTo(p.x, p.y);
...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const type = () => {
      ...
      if (deleting) { el.textContent = current.substring(0, ci - 1); ci--; speed = 25; }
      else { el.textContent = current.substring(0, ci + 1); ci++; speed = 50; }
  ...
  const handleNameComplete = useCallback(() => setNameComplete(true), []);
          ...
          <div>
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="font-mono text-[0.6rem] tracking-[0.25em] uppercase text-accent">SECURITY ANALYST</span>
              </div>
              ...
              <>
                <div className="mb-6">
                  <GlitchText intensity="soft">
                    <p className="font-mono text-[0.7rem] tracking-[0.2em] uppercase text-text-muted">{content.hero.tagline}</p>
                  </GlitchText>
                </div>
                <p className="text-[0.95rem] leading-relaxed text-text-secondary mb-8 max-w-[480px]">{content.hero.desc}</p>
                <div className="flex gap-4">
                  <MagneticButton href={`mailto:${content.hero.email}`}>
                    <span className="font-mono text-[0.7rem] tracking-widest uppercase px-6 py-2.5 border border-accent text-text hover:bg-accent-soft transition-colors">Email</span>
                  </MagneticButton>
                  <MagneticButton href={content.hero.linkedin}>
                    <span className="font-mono text-[0.7rem] tracking-widest uppercase px-6 py-2.5 border border-border-medium text-text-secondary hover:border-accent hover:text-text transition-colors">LinkedIn</span>
                  </MagneticButton>
                </div>
              </>
            )}
            ...
            <div className="grid grid-cols-2 gap-px bg-border border border-border">
              {content.hero.indicators.map((ind) => (
                <div key={ind.label} className="flex flex-col gap-1 p-4 bg-bg hover:bg-bg-elevated transition-colors group">
                  <span className="font-mono text-[0.5rem] tracking-[0.15em] uppercase text-text-muted group-hover:text-text-secondary transition-colors">{ind.label}</span>
                  <span className={`font-mono text-[0.75rem] ${ind.accent ? 'text-accent' : 'text-text-secondary'}`}>{ind.value}</span>
                </div>
              ))}
              <div className="flex flex-col gap-1 p-4 bg-bg hover:bg-bg-elevated transition-colors col-span-2">
                ...
                <span className="font-mono text-[0.75rem] text-text-secondary min-h-[1.2em]">
                  <span ref={typewriterRef} /><span className="animate-pulse text-accent">|</span>
                </span>
              </div>
            </div>
